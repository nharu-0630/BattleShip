...
class Algorithm017 extends Interface {
    ...
    private int attackBeforeValue = -1;
    ...
    public void Think() {
        ...
        if (IsAllyLastMove()) {
            ...
            for (int x = 0; x < Board.BOARD_SIZE; x++) {
                for (int y = 0; y < Board.BOARD_SIZE; y++) {
                    Point point = (new Point(x, y)).Plus(AllyLastMoveVector());
                    ...
                    if (AllyLastMoveVector().Distance() == 2) {
                        if ((new Point(x, y).Plus(AllyLastMoveVector()).Divide(2)).IsRange()) {
                            if (Board.GetCell(new Point(x, y).Plus(AllyLastMoveVector()).Divide(2))
            ...
            switch (AllyLastMoveVector().x) {
                case 2:
                case 1:
                    for (int x = AllyLastMoveVector().x; x < Board.BOARD_SIZE; x++) {
                        for (int y = 0; y < Board.BOARD_SIZE; y++) {
                ...
                case -1:
                case -2:
                    for (int x = 0; x < Board.BOARD_SIZE + AllyLastMoveVector().x; x++) {
                        for (int y = 0; y < Board.BOARD_SIZE; y++) {
                            if (excludePoints.contains(new Point(x, y))) {
                                continue;
                            }
            ...
            switch (AllyLastMoveVector().y) {
                case 2:
                case 1:
                    for (int x = 0; x < Board.BOARD_SIZE; x++) {
                        for (int y = AllyLastMoveVector().y; y < Board.BOARD_SIZE; y++) {
                            if (excludePoints.contains(new Point(x, y))) {
                                continue;
                            }
                            int value = Board.GetCell(x, y).GetValue(alphaSide, 1);
                ...
                case -2:
                    for (int x = 0; x < Board.BOARD_SIZE; x++) {
                        for (int y = 0; y < Board.BOARD_SIZE + AllyLastMoveVector().y; y++) {
                            if (excludePoints.contains(new Point(x, y))) {
                                continue;
                            }
                            int value = Board.GetCell(x, y).GetValue(alphaSide, 1);
        ...
        if (IsEnemyLastMove()) {
            ...
            for (int x = 0; x < Board.BOARD_SIZE; x++) {
                for (int y = 0; y < Board.BOARD_SIZE; y++) {
                    Point point = (new Point(x, y)).Plus(EnemyLastMoveVector());
                    ...
                    if (EnemyLastMoveVector().Distance() == 2) {
                        if ((new Point(x, y).Plus(EnemyLastMoveVector()).Divide(2)).IsRange()) {
                            if (Board.GetCell(new Point(x, y).Plus(EnemyLastMoveVector()).Divide(2))
        ...
        if (IsAllyLastAttack()) {
                ...
                if (Board.IsLastMove(!alphaSide)) {
                    ...
                    if (estimatedPoint.IsRange()) {
                        ...
                        if (Board.IsEnableAttackPoint(alphaSide, estimatedPoint)) {
                            ...
                            attackBeforeValue = Board.GetCell(estimatedPoint).GetValue(alphaSide, 0);
                    ...
                    if (Board.IsEnableAttackPoint(alphaSide, AllyLastAttackPoint())) {
                        attackBeforeValue = Board.GetCell(AllyLastAttackPoint()).GetValue(alphaSide, 0);
            ...
            if (!AllyLastAttackResult().contains(Board.ATTACK_HIT)
                    && !AllyLastAttackResult().contains(Board.ATTACK_SINK)) {
                if (attackBeforeValue == 20 && EnemyLastMoveVector() != null) {
                    Point point = AllyLastAttackPoint().Minus(EnemyLastMoveVector());
                    if (point.IsRange()) {
                        Board.GetCell(point).SetValue(alphaSide, 0, 20);
                ...
                if (estimatedAttackedFlag) {
                    ...
                    if (Board.IsEnableAttackPoint(alphaSide, estimatedBeforePoint)) {
                        attackBeforeValue = Board.GetCell(estimatedBeforePoint).GetValue(alphaSide, 0);
        ...
        if (prepareTurned && Board.IsEnableAttackPoint(alphaSide, preparePoint)) {
            prepareTurned = false;
            attackBeforeValue = Board.GetCell(preparePoint).GetValue(alphaSide, 0);
        ...
        if (Board.GetMaxValuePoints(alphaSide, true, 0).size() != 0) {
            Point point = Board.GetRandomPoint(Board.GetMaxValuePoints(alphaSide, true, 0));
            attackBeforeValue = Board.GetCell(point).GetValue(alphaSide, 0);
            DoAttack(point);
            return;
        } else {
