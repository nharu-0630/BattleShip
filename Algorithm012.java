...
class Algorithm012 extends Interface {
    Algorithm012(boolean alphaSide, boolean isEnemySecret) {
            ...
            if (Board.GetLastAttackResult(!alphaSide).contains(2)) {
                ...
                if (Math.random() < 0.6) {
                    ...
                    for (Point point : Board.GetShipPoints(alphaSide)) {
                        if (Board.GetFilterMoveEnablePoints(alphaSide, point, Board.GetCrossPoints(point, 1, 2))
                                .size() != 0) {
                            DoMove(point, Board.GetRandomPoint(
                                    Board.GetFilterMoveEnablePoints(alphaSide, point,
                                            Board.GetCrossPoints(point, 1, 2))));
                            return;
                        }
                    }
                }
